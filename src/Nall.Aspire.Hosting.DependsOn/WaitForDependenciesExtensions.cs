...
public static class WaitForDependenciesExtensions
{
        ...
        where T : IResource
    {
        ...
        return builder.WithAnnotation(
            new WaitOnAnnotation(other.Resource) { WaitUntilCompleted = true }
        );
    }
    ...
    private static IDistributedApplicationBuilder AddWaitForDependencies(
    ...
    private class WaitForDependenciesRunningHook(
    ...
    ) : IDistributedApplicationLifecycleHook, IAsyncDisposable
    {
        ...
        )
        {
            ...
            var waitingResources =
                new ConcurrentDictionary<
                    IResource,
                    ConcurrentDictionary<WaitOnAnnotation, TaskCompletionSource>
                >();

            // For each resource, add an environment callback that waits for dependencies to be running
            foreach (var r in appModel.Resources)
            {
                var resourcesToWaitOn = r
                    .Annotations.OfType<WaitOnAnnotation>()
                    .ToLookup(a => a.Resource);
            ...
            _ = Task.Run(
                this.ExecuteStateMachineAsync(resourceNotificationService, waitingResources),
                cancellationToken
            );
        ...
        private static async Task BuildDependenciesGraphAsync(
            ResourceNotificationService resourceNotificationService,
                    ...
                    foreach (var waitOn in group)
                    {
                        ...
                        async Task Wait()
                        {
                            context.Logger?.LogInformation(
                                ...
                                waitOn.Resource.Name
                            );

                            await tcs.Task;

                            context.Logger?.LogInformation(
                                "Waiting for {Resource} completed.",
                                waitOn.Resource.Name
                            );
                        }
        ...
        private Func<Task?> ExecuteStateMachineAsync(
            ...
            async () =>
            {
                ...
                await foreach (
                    var resourceEvent in resourceNotificationService.WatchAsync(stoppingToken)
                )
                {
                    if (
                        waitingResources.TryGetValue(
                            resourceEvent.Resource,
                            out var pendingAnnotations
                        )
                    )
                    {
                        foreach (var (waitOn, tcs) in pendingAnnotations)
                        {
                            ...
                            else if (waitOn.States is null)
                            {
                                ...
                                else if (IsKnownTerminalState(resourceEvent.Snapshot))
                                {
                                    ...
                                    tcs.TrySetException(
                                        new AspireHostException(
                                            $"Dependency {waitOn.Resource.Name} failed to start"
                                        )
                                    );
                                }
                            }
        ...
        private async Task DoTheHealthCheck(ResourceEvent resourceEvent, TaskCompletionSource tcs)
        {
            ...
            var operation = await ConstructHealthCheck(
                logger,
                tcs,
                resource,
        ...
        )
        {
            ...
            if (healthCheckAnnotation?.HealthCheckFactory is { } factory)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    logger.LogError(
                        ex,
                        "Failed to construct a health check - {Name}",
                        resource.Name
                    );
